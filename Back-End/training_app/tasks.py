...
def send_training_mail(serialized_training_data):
    try:
        ...
        schools_id = [training['school']
                      for training in serialized_training_data["trainingDetail"]]

        trainer_email = User.objects.get(id=trainer_id).email
        schools_name_objects = School.objects.filter(
            id__in=schools_id).values_list("name", flat=True)
...
@shared_task(name="send_report_to_each_school_principle")
def send_report(serialized_data):
    pass
